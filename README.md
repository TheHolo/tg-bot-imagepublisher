...
Мастер также можно открыть кнопкой `➕ Добавить публикацию` или командой `/new`. Пользовательская подпись вводится как обычный текст, экранируется перед отправкой и может содержать до 1024 символов. Кнопка `Использовать автоподпись` возвращает подпись, собранную из шаблона канала, метаданных и тегов.
