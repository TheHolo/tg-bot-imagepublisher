...
Команды: `/start`, `/help`, `/status <id>`, `/queue [alias]`, `/preview [id]`, `/publish [id]`, `/cancel <id>`, `/retry <id>`, `/recent`, `/channels`, `/channel_interval <alias> <30s|15m|2h|1d|0>`, `/providers`, `/stats`, `/health`. `/queue` показывает активные задания всех каналов с alias и расчётным временем отправки, а `/queue <alias>` — очередь отдельного канала. Расчёт учитывает независимый интервал канала, позицию job, retry и ручной приоритет; скачивание и занятость worker могут добавить фактическую задержку. `/preview` без ID показывает в текущем чате самое старое задание `queued`, а `/preview <id>` — выбранное задание. Под предпросмотром можно опубликовать пост сейчас, заменить или добавить пользовательские теги и отменить публикацию; сам просмотр не меняет очередь и интервал. `/publish` без ID приоритетно публикует самое старое задание `queued`, а `/publish <id>` — выбранное; текущую активную операцию команда не прерывает. После успешной ручной публикации интервал канала начинается заново от фактического времени отправки. Интервал задаётся и отсчитывается отдельно для каждого канала; `0` отключает задержку. Команды и callback-кнопки доступны только ID из `ADMIN_IDS`.
