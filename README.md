...
- редактируемый предпросмотр с изменением заголовка, описания, тегов, канала и итоговой подписи;
- управление каналами, очередью, точным временем, массовой отменой и повтором ошибок;
...
| `timezone` | Часовой пояс IANA для точного времени в интерфейсе, например `Asia/Vladivostok`. |
...
Самый простой способ добавить публикацию — отправить боту одну или несколько ссылок без дополнительных параметров. Бот запустит пошаговый мастер:

1. получит данные публикаций;
2. предложит выбрать целевой канал кнопкой;
3. покажет итоговую подпись и позволит изменить её перед подтверждением.

Мастер также можно открыть кнопкой `➕ Добавить публикацию` или командой `/new`. На экране подтверждения можно изменить заголовок, описание, теги, канал и итоговую подпись, а кнопка `Показать медиа` отправляет облегчённые изображения для проверки. Для Pixiv и DeviantArt используются `preview_url`; при фактической публикации worker по-прежнему скачивает оригиналы.

Пользовательская подпись вводится как обычный текст, экранируется перед отправкой и может содержать до 1024 символов. Кнопка `Использовать автоподпись` возвращает подпись, собранную из шаблона канала, отредактированных метаданных и тегов.

Для опытных пользователей сохранён быстрый режим без мастера.
...
- `➕ Добавить публикацию` — пошагово запросить ссылки, выбрать канал и подготовить подпись;
- `📋 Очередь` — экран управления общей очередью или очередью выбранного канала;
- `🖼 Следующий пост` — выбор ближайшего поста во всей очереди или в активном канале;
- `📊 Статистика` — текущая статистика заданий;
- `📡 Каналы` — состояние, права бота и управление каждым зарегистрированным каналом;
...
Список активных каналов в подменю очереди и предпросмотра строится из текущего состояния БД. Любой inline-список каналов показывает не более 10 каналов на странице; при необходимости появляются кнопки перехода на предыдущую и следующую страницы. Главное меню не заменяет текстовый ввод: ссылку на публикацию по-прежнему можно отправить обычным сообщением.

Команды:

- `/start`, `/menu` — открыть главное меню;
- `/new` — запустить пошаговое создание публикации;
...
После выбора канала экран показывает число ожидающих и обрабатываемых заданий, ближайшую публикацию, режим `photo`/`document`/`auto`, интервал, активное задание, число подписчиков с изменением за 24 часа, 7 и 30 дней, а также результат проверки права бота публиковать сообщения.

Число подписчиков обновляется вручную при каждом открытии или обновлении карточки канала. В фоне бот сохраняет по одному успешному автоматическому снимку для каждого зарегистрированного канала в календарный день часового пояса `timezone`. При ошибке бот отправляет администраторам из `ADMIN_IDS` название канала и причину, затем повторяет попытку каждые 5 минут до успеха; повторные уведомления одной и той же дневной ошибки не дублируются. Снимки старше 400 дней автоматически удаляются. Исторические изменения появятся после накопления данных за соответствующий период.

Доступные действия:

- приостановить или возобновить автоматическую обработку канала;
- изменить интервал без перезапуска приложения;
- сделать канал основным для новых администраторов и fallback-выбора;
- немедленно отправить следующее задание, в том числе во время паузы;
- обновить состояние и права.

Пауза не отключает канал и не отменяет задания: новые публикации можно продолжать добавлять, а worker возобновит очередь после снятия паузы. Ручная публикация обходит паузу и текущий интервал. Изменённые через интерфейс интервал и основной канал сохраняются в БД и не сбрасываются при перезапуске; значения `CHANNELS_JSON` используются для их первоначальной настройки.

### Управление очередью

Экран очереди поддерживает фильтры `Активные`, `Ожидают`, `В работе`, `Ошибки`, `Готово` и `Отменено`. Карточка задания позволяет:

- переместить ожидающее задание выше или ниже внутри его канала;
- назначить или сбросить точное время публикации в часовом поясе `timezone`;
- опубликовать задание немедленно;
- показать облегчённое медиа;
- отменить или повторить задание.

Массовая отмена действует только на задания текущего фильтра и всегда требует подтверждения. Кнопка массового повтора возвращает в очередь обычные неудачные задания, но пропускает `uncertain_publish`: перед их ручным повтором необходимо проверить канал, иначе возможен дубль.
