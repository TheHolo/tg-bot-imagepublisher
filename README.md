...
После успешного запуска отправьте боту `/health`, затем `/channels`. Если бот не отвечает, сначала проверьте журнал systemd, `BOT_TOKEN` и наличие вашего ID в `ADMIN_IDS`.
...
Для переноса уже работающей systemd-установки в окончательный корень используйте отдельную [инструкцию миграции](docs/server-root-migration.md). В ней предусмотрены проверка ветки, пересоздание virtualenv, сохранение исходной установки и полный откат.
